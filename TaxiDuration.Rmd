...
```{r}
# Weibull: controlling for distance 
#month_weib = survreg(Surv(trip_duration) ~ as.factor(month) + total_distance, dist = "weibull", data = TaxiRoutes)
#summary(month_weib)

#Km models
KM_month2 <- survfit(Surv(trip_duration) ~ as.factor(month) + total_distance, data = TaxiRoutes)
#plot(KM_month, col = c('red', 'orange', 'gold', 'green', 'cyan', 'purple'), xlim = c(0, 5000), xlab = "Trip Duration (seconds)", ylab = "Survival Estimate", main = "Taxi Trip Duration by Month")
#legend("topright", inset = 0.05, title = "Month", legend = c("1", "2", "3", "4", "5", "6"), fill = c('red', 'orange', 'gold', 'green', 'cyan', 'purple'))
```
...
summary(survreg(Surv(trip_duration) ~ weather+distance_traveled+factor(passenger_count), data = TaxiWeather), dist = "lognormal")
```
...
RoadsNameData <- sapply(RoadsNameData, function(x) sub("(?<=3).*// ", "", x, perl=TRUE))
RoadsNameData <- sapply(RoadsNameData, function(x) sub("ST", "", x))
RoadsNameData <- sapply(RoadsNameData, function(x) sub("ND", "", x))
RoadsNameData <- sapply(RoadsNameData, function(x) sub("RD", "", x))
RoadsNameData <- sapply(RoadsNameData, function(x) sub("REET", "STREET", x))
...
for (i in 1:nrow(RoadsNameData)){
  bikecount = 0
  BikeLaneTripList = vector(,nrow(RoadsNameData))
  for (j in (2:ncol(RoadsNameData)){
    if (RoadsNameData[i,j] != "NA"){
      for (k in 1:nrow(StreetInfo){
        if(StreetInfo[k, 3] == RoadsNameData[i,j]){
...
# Exponential
pcount_exp <- survreg(Surv(trip_duration) ~ as.factor(passenger_count), data = TaxiRoutes, dist = "exponential")
summary(pcount_exp)

# Weibull
pcount_weib <- survreg(Surv(trip_duration) ~ as.factor(passenger_count), data = TaxiRoutes, dist = "weibull")
summary(pcount_weib)

#Lognormal
# Exponential
pcount_lnorm <- survreg(Surv(trip_duration) ~ as.factor(passenger_count), data = TaxiRoutes, dist = "lognormal")
