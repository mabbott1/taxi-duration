...
Hi i'm Katja and I contributed!
...
## Data Cleaning:
### this should work but my  R kept crashing
```{r}
...
# Restricts duration to under 80000 seconds (about 22 hours) and over 20 seconds
cleanTaxi3 <- cleanTaxi2 %>%
  filter(trip)


# if you figure out how, save the data so we don't have to do cleaning each time
#write.csv(cleanTaxi3, file = "tempTaxi.csv")



# make another map
ggplot(data = states) + geom_polygon(aes(x = long, y = lat, fill = region, group = group), color = "white") + coord_fixed(1.3) + guides(fill=FALSE) + geom_point(data = cleanTaxi3, aes(x = dropoff_longitude, y = dropoff_latitude), color = "black", size = 2, alpha = 0.5) 
