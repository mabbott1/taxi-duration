...
```{r}
WeatherData <-read_csv("WeatherTaxi.csv")
...
To look at accidents, we will first find shortest road routes between start and end.
Then look to see if there was an accident in that route anytime between the start and end time of the trip.
If true, then add an indicator variable for accident.

```{r}
Accidents <- read.csv("accidents_2016.csv")
#View(Accidents)

Foo <- read.csv("fastest_routes_train_part_1.csv")
dim(Foo)
