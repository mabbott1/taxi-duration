...
## NEW: try fitting survival curve with weibull distribution
```{r}
...
### Trip duration by rush hour, splitting rush hour into more categories


```{r}
CreateRushMorn <- function(data1,timeCat){
  rushMorn = c( 1:nrow(data1))
  for(i in 1:nrow(data1)){
    if(hour(timeCat[i]) <= 8 && hour(timeCat[i]) >= 6 ){
      rushMorn[i] = 1
    }
  ...
  return(newdata1 = (cbind(data1, rushMorn)))
}
cleanTaxi2 = CreateRushMorn(cleanTaxi, cleanTaxi$pickup_datetime)


CreateRushEve <- function(data1,timeCat){
  rushEve = c( 1:nrow(data1))
  for(i in 1:nrow(data1)){
    if(hour(timeCat[i]) >= 15 && hour(timeCat[i]) <= 17){
      rushEve[i] = 1
    }
    else{
      rushEve[i] = 0
  ...
  return(newdata1 = (cbind(data1, rushEve)))
}

cleanTaxiFin = CreateRushEve(cleanTaxi2, cleanTaxi$pickup_datetime)
...
```{r}
KMRush2 = survfit(Surv(cleanTaxiFin$trip_duration)~cleanTaxiFin$rushMorn+cleanTaxiFin$rushEve, conf.type = "plain")

plot(KMRush2, col = c("black", "red", "green"), xlim= c(0,5000), xlab = "Trip Duration (seconds)", ylab = "Survival Estimate", main = "Taxi Trip Duration by Rush Hour")
legend("topright", inset = 0.05, title = "Rush Hour Indicator", legend = c("0", "1", "other"), fill = c('black', 'red', 'green'))
...
* looking at bike lanes, busy streets...

First find shortest road routes between start and end.
...
```{r}
miniTaxi <- head(TaxiRoutes, 20)
miniTaxiNew <- miniTaxi %>%
  separate(split.col = street_for_each_step, into = paste("street", fill = T, sep = '|'))
